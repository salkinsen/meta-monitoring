...
echo "----------------SCENARIO 8 STARTING----------------" | tee -a $outfile

./deploy.sh -r $outfile -c $loadgen_csv \
    -m ../../microservices-demo/kubernetes-manifests/microservices-tracing-sampling-1perc/ \
...
echo "----------------SCENARIO 8 FINISHED----------------" | tee -a $outfile